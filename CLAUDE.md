# CLAUDE.md

This file provides guidance to Claude Code (claude.ai/code) when working with code in this repository.

## Project Overview

ESP32 Sensor Device Status Dashboard — a live fleet-operations dashboard for monitoring firmware status across ~18 ESP32 IoT sensor devices. Tracks device-reported firmware versions, deployment status, and OTA history. Firmware deploys are performed per-device from the console.

## Tech Stack

- **Frontend:** Vanilla HTML/CSS/JavaScript in a single `index.html` (~6,900 lines). No framework, no bundler, no build step.
- **Data Pipeline:** Python 3.12+ scripts using only stdlib. No pip dependencies.
  - `scripts/build_dashboard_data.py` — fetches live device telemetry from the monitoring API, writes `data/dashboard-data.json`
  - `scripts/check_fleet_alerts.py` — evaluates the behind-device rule, creates GitHub issues
  - `scripts/update_device_registry.py`, `scripts/backfill_version_history.py` — utility scripts
- **CI/CD:** Two GitHub Actions workflows — `update-dashboard-data.yml` (data pipeline + fleet alerts, every 6 hours + manual dispatch) and `tests.yml` (unittest on push/PR to `main`).
- **Hosting:** GitHub Pages (static).

## Commands

No build system. The dashboard is served as static HTML.

```bash
# Run the data pipeline locally (requires API credentials)
python scripts/build_dashboard_data.py

# Serve locally for development
python -m http.server 8000

# Run all tests
python -m unittest discover -s tests -v
```

Environment variables for scripts:
- `API_USERNAME`, `API_PASSWORD` — monitoring API credentials
- `SENSORHUB_REPO_PAT` (optional) — GitHub token for firmware repo metadata
- `GITHUB_TOKEN` (in Actions) — for fleet alerts (GitHub issues)

## Architecture

### Data Flow

1. **Live Telemetry (every 6 hours):** GitHub Actions → `scripts/build_dashboard_data.py`
   - Fetches device list from `GET /devices?limit=500` (device-reported version, battery_level)
   - Falls back to per-device `GET /devices/{mac}/logs/latest` if list fetch fails
   - Fetches OTA history from `GET /devices/{mac}/events` and filters client-side for OTA-completed events (current API code 117; older fixtures used 119)
   - Merges with `data/devices.json` registry (fallback for offline/unknown devices)
   - Derives **pending OTA** from the device-scoped console firmware record (`GET /firmwares/latest?deviceMac=`) vs the device-reported version — not from the registry and not from the fleet track target. Pending clears automatically once the device reports the target version.
   - **Reconciles the registry from telemetry** (`reconcile_registry_from_telemetry`): when a device reports a firmware newer than `installed_firmware_version`, the pipeline writes the new version + `last_firmware_update` back into `data/devices.json` (forward-only, never downgrades) and drops obsolete `pending_ota_*` fields. This is what keeps a completed OTA from showing a permanent version mismatch. The Actions workflow commits `devices.json` so this persists.
   - Outputs `data/dashboard-data.json` with per-device telemetry, capabilities, and version state

2. **Frontend Initialization:** `index.html` loads `dashboard-data.json`, `devices.json`, and `version-changes.json` via fetch

3. **Fleet Alerts:** After data pipeline, GitHub Actions runs `scripts/check_fleet_alerts.py`
   - Evaluates rules against `dashboard-data.json` and `version-changes.json`
   - Creates/closes GitHub issues for behind devices

### Frontend State

All UI state lives in `dashboardState` object in `index.html`. Key rendering functions:
- `initializeDashboard()` — entry point, parallel data fetching, state initialization
- `renderDeviceCards()` / `renderDeviceTable()` — card vs. list views
- `renderVersionHistory()` — interactive version timeline
- `renderStatusBreakdown()` — Chart.js doughnut chart
- `applyFiltersAndRender()` — orchestrates filtering, sorting, re-rendering
- Device drawer renderer for detailed telemetry and OTA history

### Device Status Logic

Status determined by comparing device firmware to track target:
- **Up to date** — version equals target_version
- **Patch available** — same major.minor, behind on patch
- **Needs update** — behind on major or minor
- **In development** — track has no target version
- **Not deployed** — device has no version record
- **Unknown** — version data missing or indeterminate

Version source of truth: device-reported version (from API) is preferred; registry fallback when offline.

### Key Data Files

- `data/devices.json` — device registry (MACs, labels, hardware, capabilities, registry version fallback). Hand-edit to add/remove devices or update manual data. **Also pipeline-written:** `build_dashboard_data.py` advances `installed_firmware_version` / `last_firmware_update` from confirmed device telemetry (forward-only) and the workflow commits the result, so the recorded version self-heals after an OTA without manual edits. `pending_ota_version` / `pending_ota_created` are **no longer stored here** — pending is derived live from the console firmware record (deploy scripts must not write them back).
- `data/version-changes.json` — release history with component tags. **Generated, not hand-edited:** produced by `scripts/generate-status-json.sh` in the firmware repo from git **tag annotations** — each entry's `description` is the first line of the tag's annotation message (`git tag -a -m "..."`). Components are grepped from the annotation body. Committed here via "Update version changes data" commits. To change a description, fix the firmware repo's tag annotation and regenerate (or edit this file directly as a one-off).
- `data/dashboard-data.json` — generated by pipeline every 6 hours; GitHub Actions commits it. Contains live telemetry + version state.
- `data/version-history.json` — historical per-device version records. Snapshotted on every pipeline run by `build_dashboard_data.py` (`upsert_version_history_snapshot`); also seedable via `backfill_version_history.py`. Committed by the workflow alongside `dashboard-data.json`.

**.gitignore state:** All four `data/*.json` files are **tracked and committed**. `.gitignore` ignores only Python artifacts (`__pycache__/`, `*.py[cod]`, etc.).

## API Endpoints

Base: `https://monitoring-api.3aentreprise.com`

**Device telemetry:**
- `GET /devices?limit=500` — device list with lastLog (firmwareVersion, createdAt, batteryLevel), isOnline, lastReportedAt
- `GET /devices/{mac}/logs/latest` — latest log for one device (fallback if list fails)
- `GET /devices/{mac}/events` — event history (returns `{events: [...]}` or a bare list; Bearer token required, rejects Basic). Filtered client-side: OTA Update Completed = OTA history, System Startup = reported version

**Firmware management:**
- `GET /firmwares/latest?deviceMac={mac}` — latest available firmware for device
- `GET /firmwares?limit=500` — list all firmware records (filter client-side by versionCode)
- `POST /firmwares` — create a device-scoped firmware record

**Authentication:**
- `POST /auth/token` — form fields `username`, `password` → Bearer JWT (preferred)
- Fallback: HTTP Basic Auth with same credentials

Auth strategy: scripts try JWT first; on failure use Basic Auth.

## Scripts Overview

- **`build_dashboard_data.py`** — main pipeline. Fetches live telemetry + history, merges with registry, emits dashboard JSON.
- **`check_fleet_alerts.py`** — evaluates alert rules, creates/closes GitHub issues. `--dry-run` (default) or `--execute`.
- **`update_device_registry.py`** — utility to bulk-update `devices.json` fields.
- **`backfill_version_history.py`** — populates `version-history.json` from existing version-changes.

## Tests

Test files in `tests/`:
- `test_build_dashboard_data.py` — pipeline logic (version merging, telemetry extraction, OTA history, fallback behavior)
- `test_fleet_alerts.py` — alert rule evaluation (behind threshold)

All tests use monkeypatched network access (no real API calls). Run with `python -m unittest discover -s tests -v`.

## Sensor Component Types

Tags used in version-changes.json and device tracking: `core`, `OneWire`, `MODBUS`, `digital`, `switch`, `virtual`, `algae`.

## Conventions

- All styles and scripts are embedded in `index.html` (no external JS/CSS files).
- XSS prevention: device names and locations escaped via `escapeHtml()`.
- Version strings normalized with `v` prefix (e.g., `v3.7.1`); comparison via `version_tuple()` for semver logic.
- Snake_case in JSON output and Python (`reported_version`, `ota_history`, `pending_ota_version`); camelCase only in frontend state.
- CDN dependencies: Bootstrap 5.2.3, Bootstrap Icons, Chart.js, Google Fonts (JetBrains Mono, Noto Sans).
- Dark theme with blue accents; CSS custom properties for theming.
- Firmware deployment scripts live in `glaecier-sensorhub-data-collector/scripts/` (build, upload, OTA management).
